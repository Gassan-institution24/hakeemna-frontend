...
const Receiptpaymentvoucher = (props) => {
    ...
    <div>
      {isAdding && (
        <AddModel
          textDetails={[
            {
              name: "couta_serial_num",
              nameShown: "couta serial num",
            },
            {
              name: "date",
              nameShown: "date",
      ...
      {data?.map((one, idx) => {
        return (
          <ModelCard
            ...
            one={one}
            path={"receiptpayment"}
            h2items={["couta_serial_num", "date", "unit_service"]}
          />
        );
      })}
...
const mapStateToProps = (state) => ({
  ...
  export default connect(mapStateToProps, mapDispatchToProps)(Receiptpaymentvoucher);
