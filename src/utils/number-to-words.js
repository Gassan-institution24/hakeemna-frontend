...
const numberToArabicWords = (number) => {
    ...
    // eslint-disable-next-line
    if (number < 100) return tens[Math.floor(number / 10)] + (number % 10 !== 0 ? ' و' + units[number % 10] : '');

    if (number < 1000) {
        // eslint-disable-next-line
        return hundreds[Math.floor(number / 100)] + (number % 100 !== 0 ? ' و' + numberToArabicWords(number % 100) : '');
    }

    if (number < 10000) {
        // eslint-disable-next-line
        return thousands[Math.floor(number / 1000)] + (number % 1000 !== 0 ? ' و' + numberToArabicWords(number % 1000) : '');
    }
