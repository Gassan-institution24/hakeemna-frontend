...
import { Box, Button, Dialog, Divider, MenuItem, Typography, DialogTitle, DialogActions, DialogContent } from '@mui/material';
...
import { useGetAppointmentTypes, useGetUSActiveWorkGroups, useGetUSActiveWorkShifts, useGetUSActiveServiceTypes } from 'src/api';
...
export default function CountriesTableRow({ row, selected }) {
  ...
  const renderPrimary = (
    <TableRow hover selected={selected}>
      ...
      <TableCell align="center">
        <Button variant='contained' onClick={newVisit.onTrue} color='secondary' sx={{ fontSize: 13, borderRadius: 0, p: 0.2 }}>{t('new appointment')}</Button>
      </TableCell>
    </TableRow>
  );
  ...
  const defaultValues = {
    ...
    work_shift: workShiftsData.filter((one) => {
      ...
      if (startTime.getTime() <= endTime.getTime()) {
        return currentDate.getTime() >= startTime.getTime() && currentDate.getTime() < endTime.getTime();
      }
      // If the shift crosses midnight
      const endTimeNextDay = new Date(endTime.getTime() + 24 * 60 * 60 * 1000);
      return currentDate.getTime() >= startTime.getTime() || currentDate.getTime() < endTimeNextDay.getTime();
    })?.[0]?._id,
    service_types: [],
  };
  ...
  return <>
    ...
    <Dialog open={newVisit.value} onClose={newVisit.onFalse} >
      <FormProvider methods={methods} onSubmit={onSubmit}>
        <DialogTitle sx={{ mb: 2, textTransform: 'capitalize' }}>{t('new appointment')}</DialogTitle>
        <DialogContent>
