...
export default function CreateMonthlyReport({
  ...
  const defaultValues = {
    ...
    calculated_time:
      row?.calculated_time ||
      (monthly ? yearlyIntervalData?.calculated_time : intervalData?.calculated_time) ||
      0,
    annual:
      row?.annual || (monthly ? yearlyIntervalData?.annual : intervalData?.annual) || annual || 0,
    sick: row?.sick || (monthly ? yearlyIntervalData?.sick : intervalData?.sick) || sick || 0,
    unpaid:
      row?.unpaid || (monthly ? yearlyIntervalData?.unpaid : intervalData?.unpaid) || unpaid || 0,
    public:
      row?.public ||
      (monthly ? yearlyIntervalData?.public : intervalData?.public) ||
      publicHolidays ||
      0,
    other: row?.other || (monthly ? yearlyIntervalData?.other : intervalData?.other) || other || 0,
    social_security:
      row?.social_security ||
      (monthly ? yearlyIntervalData?.social_security : intervalData?.social_security) ||
      0,
    tax: row?.tax || (monthly ? yearlyIntervalData?.tax : intervalData?.tax) || 0,
    deduction:
      row?.deduction || (monthly ? yearlyIntervalData?.deduction : intervalData?.deduction) || 0,
    total: row?.total || (monthly ? yearlyIntervalData?.total : intervalData?.total) || 0,
    annual_equivalent:
      row?.annual_equivalent ||
      (monthly ? yearlyIntervalData?.annual_equivalent : intervalData?.annual_equivalent) ||
      0,
    sick_equivalent:
      row?.sick_equivalent ||
      (monthly ? yearlyIntervalData?.sick_equivalent : intervalData?.sick_equivalent) ||
      0,
    unpaid_equivalent:
      row?.unpaid_equivalent ||
      (monthly ? yearlyIntervalData?.unpaid_equivalent : intervalData?.unpaid_equivalent) ||
      0,
    public_equivalent:
      row?.public_equivalent ||
      (monthly ? yearlyIntervalData?.public_equivalent : intervalData?.public_equivalent) ||
      0,
    other_equivalent:
      row?.other_equivalent ||
      (monthly ? yearlyIntervalData?.other_equivalent : intervalData?.other_equivalent) ||
      0,
    note: row?.note || '',
