...
export default function UserCardList() {
  ...
  const [currentTab, setCurrentTab] = useState('medicalAnalysis');
  const TABS = [
    {
  ...
  return (
    <Container>
      <CustomBreadcrumbs
        heading={t('My medical analysis')}
        links={[
          { name: t('dashboard'), href: paths.dashboard.root },
          { name: t('user'), href: paths.dashboard.user.root },
          { name: t('My medical analysis') },
        ]}
        sx={{ mb: { xs: 3, md: 5 } }}
      />
      ...
      {currentTab === 'medicalAnalysis' && (
        <Box
          ...
          sx={{ width: '100%' }}
        >
          <MedicalAnalysis user={user?.patient._id} />
        </Box>
      )}
    </Container>
  );
