  ...
  return (
    <Card {...other}>
      <Carousel ref={carousel.carouselRef} {...carousel.carouselSettings}>
        {IMGES?.map((app, index, idx) => (
          <CarouselItem key={idx} item={app} active={index === carousel.currentIndex} />
        ))}
      </Carousel>
...
function CarouselItem({ item, active }) {
  ...
  const renderImg = (
    <Image
      alt={title}
      src={IMGES[0]}
      overlay={`linear-gradient(to bottom, ${alpha(theme.palette.grey[900], 0)} 0%, ${theme.palette.grey[700]
        } 85%)`}
      sx={{
        width: 1,
        height: {
