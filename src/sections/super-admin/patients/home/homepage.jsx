...
export default function PatientTableView() {
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.superadmin.patients.edit(id)); /// edit
    },
    [router]
  );
  ...
  const handleShowCommunicationsRow = useCallback(
    (id) => {
      router.push(paths.superadmin.patients.communications(id)); /// edit
    },
    [router]
  );
  const handleShowFeedbacksRow = useCallback(
    (id) => {
      router.push(paths.superadmin.patients.feedback(id)); /// edit
    },
    [router]
  );
  const handleShowInsuranceRow = useCallback(
    (id) => {
      router.push(paths.superadmin.patients.insurance(id)); /// edit
    },
    [router]
  );
  const handleShowGeneralInfoRow = useCallback(
    (id) => {
      router.push(paths.superadmin.patients.info(id)); /// edit
    },
    [router]
  );
            ...
            {STATUS_OPTIONS.map((tab, idx) => (
              <Tab
                ...
                icon={
                  ...
                  >
                    {tab.value === 'all' && patientsData.length}
                    {tab.value === 'active' &&
                      patientsData.filter((order) => order.status === 'active').length}
                    {tab.value === 'inactive' &&
                ...
                .map((row) => (
                  <MobileRow
                    ...
                    fields={[
                      {
                        label: 'Code',
                        value: `${String(row.nationality?.code || '').padStart(3, '0')}-${row.sequence_number}`,
                      },
                      {
