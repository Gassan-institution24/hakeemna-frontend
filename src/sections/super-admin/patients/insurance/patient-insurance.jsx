...
export default function PatientInsuranceView({ patientData, refetch }) {
  /// edit
  ...
  const dataFiltered = applyFilter({
    inputData: patientData?.insurance,
  ...
  const handleAddRow = useCallback(
    async (id) => {
      if (patientData.insurance.some((company) => company._id === id)) {
      ...
      await axiosHandler({
        method: 'PATCH',
        path: endpoints.tables.patient(patientData?._id), /// to edit
        data: { insurance: info },
      });
      refetch();
      table.onUpdatePageDeleteRow(dataInPage?.length);
    },
    [dataInPage?.length, table, refetch, patientData?._id, patientData?.insurance]
  );
...
PatientInsuranceView.propTypes = {
