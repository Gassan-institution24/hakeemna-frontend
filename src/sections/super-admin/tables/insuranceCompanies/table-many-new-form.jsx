...
export default function NewEditManyForm() {
  ...
  const handleCreate = async () => {
    const isFormValid = data.every(
      (one) => one.name_english && one.name_arabic && one.country && one.city && one.phone
    );
    ...
    try {
      ...
      const insertedData = await axiosInstance.post(
        endpoints.insurance_companies.many,
              ...
              <TableSelectedAction
                ...
                action={
                  <>
                    ...
                    <Typography sx={{ fontSize: 14, fontWeight: 600, pr: 1 }}>City:</Typography>
                    <Select
                      variant="filled"
                      sx={{ width: '20%', border: '1px solid gray' }}
                      size="small"
