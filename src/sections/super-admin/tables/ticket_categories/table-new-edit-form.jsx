...
export default function TableNewEditForm({ currentTable }) {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      if (currentTable) {
        await axiosInstance.patch(endpoints.TicketCategories.one(currentTable._id), data);
      } else {
        await axiosInstance.post(endpoints.TicketCategories.all, data);
      }
      reset();
      enqueueSnackbar(currentTable ? 'Update success!' : 'Create success!');
      router.push(paths.superadmin.tables.ticketCategories.root);
    } catch (error) {
      console.error(error);
    }
  });
