...
import { useGetInsuranceCos } from 'src/api'; /// edit
...
export default function InsuranceCompaniesTableView() {
  ...
  const handleActivateRows = useCallback(async () => {
    ...
    table.onUpdatePageDeleteRows({
      totalRows: insuranseCosData.length,
      totalRowsInPage: dataInPage.length,
      totalRowsFiltered: dataFiltered.length,
    });
  }, [dataFiltered.length, dataInPage.length, table, insuranseCosData, refetch]);
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.superadmin.tables.insurancecomapnies.edit(id));
    },
    [router]
  );
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Insurance Companies' }, /// edit
          ]}
          action={
            <Button
              component={RouterLink}
              href={paths.superadmin.tables.insurancecomapnies.new} /// edit
              variant="contained"
            ...
            {STATUS_OPTIONS.map((tab) => (
              <Tab
                ...
                icon={
                  ...
                  >
                    {tab.value === 'all' && insuranseCosData.length}
                    {tab.value === 'active' &&
                      insuranseCosData.filter((order) => order.status === 'active').length}
                    {tab.value === 'inactive' &&
                      insuranseCosData.filter((order) => order.status === 'inactive').length}
                  </Label>
                }
              />
            ))}
            ...
            <Scrollbar>
              <Table ref={componentRef} size={table.dense ? 'small' : 'medium'}>
                ...
                <TableBody>
                  ...
                  <TableEmptyRows
                    height={denseHeight}
                    emptyRows={emptyRows(table.page, table.rowsPerPage, insuranseCosData.length)}
                  />
