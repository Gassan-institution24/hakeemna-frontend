...
import { useGetFamilyTypes, useGetRooms } from 'src/api';
...
export default function FamilyRelationTableView() {
  ...
  const { Family, loading, refetch } = useGetFamilyTypes();
  ...
  //   async (id) => {
  //     await axiosInstance.patch(
  //       `${endpoints.patients.familyType}/${id}/updatestatus`, /// edit
  //       { status: 'inactive' }
  //     );
  //     refetch();
  //     table.onUpdatePageDeleteRow(dataInPage.length);
  ...
  //     totalRows: Family.length,
  //     totalRowsInPage: dataInPage.length,
  //     totalRowsFiltered: dataFiltered.length,
  //   });
  // }, [dataFiltered.length, dataInPage.length, table, Family, refetch]);
            ...
            {STATUS_OPTIONS.map((tab, idx) => (
              <Tab
                ...
                icon={
                  ...
                  >
                    {tab.value === 'all' && Family.length}
                    {tab.value === 'active' &&
                      Family.filter((order) => order.status === 'active').length}
                    {tab.value === 'inactive' &&
                      Family.filter((order) => order.status === 'inactive').length}
                  </Label>
                }
              />
            ))}
