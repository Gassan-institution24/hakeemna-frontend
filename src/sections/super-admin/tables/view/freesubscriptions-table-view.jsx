...
export default function FreeSubscriptionTableView() {
  ...
  const dataFiltered = applyFilter({
    inputData: freeSubscriptionsData,
  ...
  const handleDownload = () => {
    ...
    saveAs(data, 'freeSubscriptionsTable.xlsx'); /// edit
  };
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.superadmin.tables.freesub.edit(id));
    },
    [router]
  );
      ...
      <Container maxWidth="xl">
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Free Subscriptions' }, /// edit
          ]}
          action={
            <Button
              component={RouterLink}
              href={paths.superadmin.tables.freesub.new} /// edit
              variant="contained"
            ...
            {STATUS_OPTIONS.map((tab, idx) => (
              <Tab
                ...
                icon={
                  ...
                  >
                    {tab.value === 'all' && freeSubscriptionsData.length}
                    {tab.value === 'active' &&
                      freeSubscriptionsData.filter((order) => order.status === 'active').length}
                    {tab.value === 'inactive' &&
                      freeSubscriptionsData.filter((order) => order.status === 'inactive').length}
                  </Label>
                }
              />
            ))}
                ...
                .map((row) => (
                  <MobileRow
                    ...
                    fields={[
                      ...
                      {
                        label: 'Specialty',
                        value: row.speciality?.name_english,
                      },
                      {
                        label: 'Date',
                        value: fDateTime(row.offer_date),
                      },
                      { label: 'Period in Months', value: row.period_in_months },
                      { label: 'Concept', value: row.concept },
                      {
                        label: 'General',
                        value: (
                          <Iconify
                            icon={!row.general ? 'mingcute:close-line' : 'eva:checkmark-fill'}
                            width={16}
                            sx={{
                              color: row.general ? 'success.main' : 'error.main',
                            }}
                          />
                        ),
                      },
