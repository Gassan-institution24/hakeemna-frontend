...
// ----------------------------------------------------------------------
const TABLE_HEAD = [
  { label: 'symbol *', width: 'calc(100%/3)' },
  { label: 'name english *', width: 'calc(100%/3)' },
  { label: 'name arabic *', width: 'calc(100%/3)' },
];
export default function NewEditManyForm() {
  ...
  const handleCreate = async () => {
    const isFormValid = data.every((one) => one.name_english && one.name_arabic && one.symbol);
    ...
    try {
      await axiosInstance.post(endpoints.measurment_types.many, data);
      router.push(paths.superadmin.tables.measurementtypes.root); /// edit
    } catch (e) {
      console.log(e);
