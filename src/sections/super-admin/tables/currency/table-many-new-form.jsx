...
// ----------------------------------------------------------------------
const TABLE_HEAD = [
  { label: 'symbol', width: 'calc(100%/4)' },
  { label: 'name english *', width: 'calc(100%/4)' },
  { label: 'name arabic *', width: 'calc(100%/4)' },
  { label: 'relation to dollar *', width: 'calc(100%/4)' },
];
export default function NewEditManyForm() {
  ...
  const handleCreate = async () => {
    const isFormValid = data.every(
      (one) => one.name_english && one.name_arabic && one.symbol && one.relation_to_dollar
    );
