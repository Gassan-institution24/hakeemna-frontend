...
import { useGetStakeholder, useGetStakeholders } from 'src/api'; /// edit
...
export default function StakeholderTableView() {
  ...
  const { stakeholdersData, loading, refetch } = useGetStakeholders();
  ...
  const handleActivate = useCallback(
    async (id) => {
      await axiosHandler({
        method: 'PATCH',
        path: `${endpoints.tables.stakeholder(id)}/updatestatus`,
        data: { status: 'active' },
      });
      refetch();
  ...
  const handleActivateRows = useCallback(async () => {
    await axiosHandler({
      method: 'PATCH',
      path: `${endpoints.tables.stakeholders}/updatestatus`,
      data: { status: 'active', ids: table.selected },
    });
    refetch();
    table.onUpdatePageDeleteRows({
      totalRows: stakeholdersData.length,
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.superadmin.stakeholders.edit(id)); /// edit
    },
    [router]
  );
  ...
  const handleShowHistoryRow = useCallback(
    (id) => {
      router.push(paths.superadmin.stakeholders.history.root(id)); /// edit
    },
    [router]
  );
  const handleShowCommunicationsRow = useCallback(
    (id) => {
      router.push(paths.superadmin.stakeholders.communications(id)); /// edit
    },
    [router]
  );
  const handleShowFeedbacksRow = useCallback(
    (id) => {
      router.push(paths.superadmin.stakeholders.feedback(id)); /// edit
    },
    [router]
  );
  const handleShowInsuranceRow = useCallback(
    (id) => {
      router.push(paths.superadmin.stakeholders.insurance(id)); /// edit
    },
    [router]
  );
  const handleShowGeneralInfoRow = useCallback(
    (id) => {
      router.push(paths.superadmin.stakeholders.info(id)); /// edit
    },
    [router]
  );
  const handleShowOffersRow = useCallback(
    (id) => {
      router.push(paths.superadmin.stakeholders.offers(id)); /// edit
    },
    [router]
  );
            ...
            {STATUS_OPTIONS.map((tab) => (
              <Tab
                ...
                icon={
                  ...
                  >
                    {tab.value === 'all' && stakeholdersData.length}
                    {tab.value === 'active' &&
                      stakeholdersData.filter((order) => order.status === 'active').length}
                    {tab.value === 'inactive' &&
            ...
            <Scrollbar>
              <Table ref={componentRef} size={table.dense ? 'small' : 'medium'}>
                ...
                <TableBody>
                  ...
                  <TableEmptyRows
                    height={denseHeight}
                    emptyRows={emptyRows(table.page, table.rowsPerPage, stakeholdersData.length)}
                  />
  ...
  if (name) {
    inputData = inputData.filter(
      (data) =>
        ...
        (data?.stakeholder_type?.name_english &&
