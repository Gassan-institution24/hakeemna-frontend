...
export default function RecordPage() {
  ...
  return (
    <Stack component={Card} spacing={3} sx={{ p: 3 }}>
      {medRecord?.map((info, i) => (
        <Box sx={{ display: 'grid', gridTemplateColumns: { md: '1fr 1fr', xs: '1fr' } }}>
          <Box>
            <Typography variant="h3">{info?.patient?.name_english}</Typography>
            <Typography sx={{ fontWeight: 600, p: 2 }}>
              {t('Date')}:&nbsp; &nbsp;
              <span style={{ color: 'gray', fontWeight: 400 }}>{fDateTime(info?.created_at)}</span>
            </Typography>
            <Typography sx={{ fontWeight: 600, p: 2 }}>
              {t('Appointment in ')}:&nbsp; &nbsp;
              <span style={{ color: 'gray', fontWeight: 400 }}>
                {info?.service_unit?.name_english}
              </span>
            </Typography>
            {info?.doctor_report?.map((doctorReport, ii) => (
              <Box>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: { md: '1fr 1fr', xs: '1fr' } }}>
            <Image src={info?.patient?.profile_picture} sx={{ m: 1 }} />
          </Box>
        </Box>
      ))}
