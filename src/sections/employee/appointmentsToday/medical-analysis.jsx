...
export default function MedicalAnalysis({ Entrance }) {
    ...
    const removeMedicalAnalysisField = (id) => {
        const filtered = medicalAnalyses.filter((item) => item.id !== id);
        setMedicalAnalyses(filtered);
    ...
    const removeMedicalAalysis = async (IdToremove) => {
        await axiosInstance.delete(endpoints.medicalAnalysisPatient.one(Entrance?.unit_service_patient,IdToremove));
    ...
    const onSubmit = async (data) => {
        try {
            ...
            const response = await axiosInstance.post(
                endpoints.medicalAnalysisPatient.all,
                payload
            );
    ...
    const watchAnalyses = methods.watch('medical_analysis');
            ...
            {medicalAnalysis?.map((info, i) => (
                ...
                >
                    {info?.medical_analysis?.map((item, index) => (
                        ...
                        >
                            <li>
                                {item?.medical_analysis?.name_english}
                            </li>
                        </ul>
                    ))}
                    <br />
                            ...
                            <div key={prescription.id}>
                                <Autocomplete
                                    ...
                                    onChange={(event, newValue) =>
                                        setValue(
                                            `medical_analysis[${index}].medical_analysis`,
                                            newValue?._id || '',
                                            { shouldValidate: true }
                                        )
