  ...
  return (
    <Card sx={{ py: 3, px: 5, mb: 2 }}>
      <Stack direction="row" justifyContent="flex-end" alignItems='center' gap={2} >
        ...
        <IconButton color="error" onClick={handleDelete}><Iconify icon='mi:delete' /></IconButton>
      </Stack>
      <Stack direction='row' gap={10} ml={1}>
        <Typography
          variant="body2"
        >{one?.adjustable_documents?.title}</Typography>
        <Typography
          variant="body2"
        >{one?.adjustable_documents?.applied}</Typography>
      </Stack>
    </Card>
  );
}
