...
export default function TableNewEditForm({ currentSelected }) {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      if(currentSelected){
        await axiosHandler({method:'PATCH',path:endpoints.tables.medfamily(currentSelected._id),data});      /// edit
      }else{
        await axiosHandler({method:'POST',path:endpoints.tables.medfamilies,data});                                  /// edit
      }
      reset();
