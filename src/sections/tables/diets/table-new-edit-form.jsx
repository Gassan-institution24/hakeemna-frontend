...
export default function TableNewEditForm({ currentSelected }) {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      if(currentSelected){
        await axiosHandler({method:'PATCH',path:endpoints.tables.diet(currentSelected._id),data:{user_modification:user._id,...data}});      /// edit
      }else{
        await axiosHandler({method:'POST',path:endpoints.tables.diets,data:{user_creation:user._id,...data}});                                  /// edit
      }
      reset();
