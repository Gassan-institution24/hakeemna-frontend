...
export default function SubSpecialtiesTableView() {
  ...
  const handleDownload = () => {
    ...
    saveAs(data, 'SubspecialtiesTable.xlsx'); /// edit
  };
      ...
      <Container maxWidth={false}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Subspecialties' }, /// edit
          ]}
          action={
            <Button
              component={RouterLink}
              href={paths.superadmin.tables.subspecialities.new} /// edit
              variant="contained"
            ...
            {STATUS_OPTIONS.map((tab) => (
              <Tab
                ...
                icon={
                  ...
                  >
                    {tab.value === 'all' && subspecialtiesData.length}
                    {tab.value === 'active' &&
                      subspecialtiesData.filter((order) => order.status === 'active').length}
                    {tab.value === 'inactive' &&
                      subspecialtiesData.filter((order) => order.status === 'inactive').length}
                  </Label>
                }
              />
            ))}
            ...
            <Scrollbar>
              <Table ref={componentRef} size={table.dense ? 'small' : 'medium'}>
                ...
                <TableBody>
                  ...
                  <TableEmptyRows
                    height={denseHeight}
                    emptyRows={emptyRows(table.page, table.rowsPerPage, subspecialtiesData.length)}
                  />
  ...
  if (name) {
    inputData = inputData.filter(
      (data) =>
        ...
        (data?.specialty?.name_english &&
          data?.specialty?.name_english.toLowerCase().indexOf(name.toLowerCase()) !== -1) ||
        (data?.specialty?.name_arabic && data?.specialty?.name_arabic.toLowerCase().indexOf(name.toLowerCase()) !== -1) ||
        data?._id === name ||
