...
export default function TableNewEditForm({ currentSelected }) {
  ...
  const defaultValues = useMemo(                                                  /// edit
    () => ({
      ...
      relation_to_dollar: currentSelected?.relation_to_dollar || '',
    }),
    [currentSelected]
    );
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      if(currentSelected){
        await axiosHandler({method:'PATCH',path:endpoints.tables.currency(currentSelected._id),data:{user_modification:user._id,...data}});      /// edit
      }else{
        await axiosHandler({method:'POST',path:endpoints.tables.currencies,data:{user_creation:user._id,...data}});                                  /// edit
      }
      reset();
      enqueueSnackbar(currentSelected ? 'Update success!' : 'Create success!');
      router.push(paths.superadmin.tables.currency.root);
