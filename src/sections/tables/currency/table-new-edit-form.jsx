...
export default function TableNewEditForm({ currentSelected }) {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      if(currentSelected){
        await axiosHandler({method:'PATCH',path:endpoints.tables.currency(currentSelected._id),data});      /// edit
      }else{
        await axiosHandler({method:'POST',path:endpoints.tables.currencies,data});                                  /// edit
      }
      reset();
