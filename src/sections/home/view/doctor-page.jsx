    ...
    return (
        <>
            <Stack gap={1} margin={5} padding={3} sx={{ backgroundColor: 'white', borderRadius: 1 }}>
                <Stack gap={3} direction={{ lg: 'row' }} justifyContent='space-between' margin={5} padding={3}>
                    <Stack direction={{ md: 'row' }} alignItems='center' justifyContent='center' gap={{ md: 10 }}>
                        ...
                        <Stack mb={3}>
                            <Typography variant='h6' mr={5} >{curLangAr ? employeeData?.employee?.name_arabic : employeeData?.employee?.name_english}</Typography>
                            <Typography variant='body2'>
                                {curLangAr ? employeeData?.employee?.speciality?.name_arabic : employeeData?.employee?.speciality?.name_english}
                            </Typography>
                            <Typography variant='body2'>
                                {curLangAr ? employeeData?.unit_service?.name_arabic : employeeData?.unit_service?.name_english}
                            </Typography>
                    ...
                    <Stack gap={1} flex={1}>
                        {employeeData?.unit_service?.address &&
                            <>
                                ...
                                <Typography variant='body2' sx={{ px: 3 }}>{employeeData?.unit_service?.address}</Typography>
                            </>
                        }
                        {employeeData?.employee?.phone &&
                            <>
                                <Stack direction='row'>
                                    <Typography variant='subtitle2' sx={{ borderBottom: '2px solid #00A76F', display: 'inline' }}>{t('phone number')}:</Typography>
                                </Stack>
                                <Typography variant='body2' sx={{ px: 3 }}>{employeeData?.employee?.phone}</Typography>
                            </>
                        }
                        {employeeData?.employee?.email &&
                            <>
                                <Stack direction='row'>
                                    <Typography variant='subtitle2' sx={{ borderBottom: '2px solid #00A76F' }}>{t('email')}:</Typography>
                                </Stack>
                                <Typography variant='body2' sx={{ px: 3 }}>{employeeData?.employee?.email}</Typography>
                            </>
                        }
                        <Stack direction='row'>
                            <Typography variant='subtitle2' sx={{ borderBottom: '2px solid #00A76F' }}>{t('insurance')}:</Typography>
                        </Stack>
                        <Stack>
                            {employeeData?.unit_service?.insurance?.map((one) => (
                                <Typography variant='body2' sx={{ px: 3 }}>{curLangAr ? one.name_arabic : one.name_english}</Typography>
                            ))}
                        </Stack>
                        <Stack direction='row'>
                            <Typography variant='subtitle2' sx={{ borderBottom: '2px solid #00A76F' }}>{t('languages')}:</Typography>
                        </Stack>
                        <Typography variant='body2' sx={{ px: 3 }}>
                            {employeeData?.employee?.languages?.map((one) => (one)).join(', ')}
                        </Typography>
                        <Stack direction='row'>
                            <Typography variant='subtitle2' sx={{ borderBottom: '2px solid #00A76F' }}>{t('certifications')}:</Typography>
                        </Stack>
                        <Stack px={3} gap={1}>
                        ...
                        <Stack direction='row'>
                            <Typography variant='subtitle2' sx={{ borderBottom: '2px solid #00A76F' }}>{t('memberships')}:</Typography>
                        </Stack>
                        <Stack px={3} gap={1}>
