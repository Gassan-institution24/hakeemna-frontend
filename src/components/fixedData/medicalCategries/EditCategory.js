...
function EditCategory(props) {
    ...
    async function submitHandler (e){
        e.preventDefault()
        await axiosHandler({setError,method:'PATCH',path:`medcategories/${props.editting}`,data:info})
        setInfo({name:'',description:''})
        props.fetchData()
    }

    useEffect(()=>{
        axiosHandler({setData:setSelected,setError,method:'GET',path:`medcategories/${props.editting}`})
    },[])
    useEffect(()=>{
