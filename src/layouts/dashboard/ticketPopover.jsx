...
export default function TicketPopover({ open, onClose }) {
  ...
  const { ticketsData, loading } = useGetTickets({ user_creation: user._id, status: "{ $in: ['pending', 'processing', 'waiting'] }" })
  ...
  useEffect(() => {
    if (!loading) {
      let currPage
      if (ticketsData.length > 1) { currPage = 2 } else if (ticketsData === 1) { currPage = 2 } else { currPage = 0 }
      setPage(currPage)
    }
  ...
  const participants = conversation
    ...
    : [];
  useEffect(() => {
    socket.on('message', (id) => { if (id === chatId) refetch() })
  ...
  return (
    <CustomPopover
      ...
      onClose={onClose}
    >
      {page === 0 && <FormProvider methods={methods} onSubmit={onSubmit}>
